...
pub mod arxml_bean {
    ...
    pub struct Cluster {
        ...
        pub frame_by_id: HashMap<i32, Frame>,
        pub frames_by_name: HashMap<String, Frame>,
    }

    impl Cluster {
        pub fn new(name:String,frames:Vec<Frame>) -> Self {
			let mut frames_id = HashMap::new();
			let mut frames_name = HashMap::new();
			let mut signals_temp = Vec::new();
			for frame in frames.iter() {
				if let Some(id) = frame.id {
					frames_id.insert(id, frame.clone());
				}
				if let Some(name) = frame.name.clone() {
					frames_name.insert(name, frame.clone());
				}
				...
				if signals.is_empty()  {
					...
					for signal in signals.iter_mut() {
						signals_temp.push(signal.clone());
					}
				}
			}
			Self {
				...
				frame_by_id: frames_id,
				frames_by_name: frames_name,
			}
		}
    }
                ...
                if is_pdu_container {
                    ...
                    while index + 4 <= data.len() {
                        ...
                        let id_bytes: [u8; 3] = data[index..index + 3]
                            ...
                            .unwrap();
                        let pdu_id =
                            u32::from_be_bytes([0u8, id_bytes[0], id_bytes[1], id_bytes[2]]);
                    ...
                    if let Some(signals) = self.signals.as_ref() {
                        ...
                        for signal in signals {
                            ...
                            // 转换为整数
                            let value = i32::from_str_radix(bits, 2)
                                .map_err(|e| PduError::InvalidBinaryString(e.to_string()))
                                .unwrap();
        ...
        pub fn encode_pdu_signals(&self, pdu: &Pdu, signals: &Vec<(String, f32)>) -> Vec<u8> {
            ...
            for (signal_name, value) in signals {
                ...
                if let Some(signal) = pdu.signals.as_ref().and_then(|signals| {
                    signals
                        .iter()
                        .find(|s| s.name.as_deref() == Some(signal_name.as_str()))
                }) {
                    let start_bit = signal.start_bit.unwrap() as usize;
                    let bit_size = signal.size.unwrap() as usize;
